...
// [[Rcpp::export]]
List MHBayesLinReg(int nsim, NumericVector theta0, Function objdens, Function proposal, NumericMatrix data, NumericVector y){
  ...
  for (int i=1; i<nsim; i++){
    ...
    while (!accept && trials<maxtrials){
      ...
      // the minus is since we used LOGS!!!!!
        if(logU <= as<double>(objdens(data, y, X)) - as<double>(objdens(data, y, theta(i-1,_)))) { 
          accept = true;
          theta(i,_) = X;
        } 
      trials++;
