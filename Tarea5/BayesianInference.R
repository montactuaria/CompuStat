...
est.sd <- sqrt(sum((samp-est.mean)^2)/(N-1)) # sd(samp)
...
#######################como no se conoce sigma, se paga el precio con una t, xk se divide sqrt(s^2)

intervals # OF THE ESTIMATOOOOOOOR!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

##########95% de las veces, la media cae en ese rango
...
data <- matrix(iris$Sepal.Width, ncol=1)   ############como se programo en C, tiene que se en forma de matriz
...
# 1)  Compute logprior es en la parte donde se debe de cambiar
cppFunction('
...
setwd("~/Dropbox/Master_Otoño2015/EstadisticaComputacional/Notas")
...
# meaning we'll keep 75% of observations to reduce autocorrelation
